...
As written in , to compute the rate of convergence we can derive an alternative faster and rapid way. Considering that
\begin{equation}
lim_{k\to \infty}\frac{|x_{k+1} - \alpha |}{|x_{k} - \alpha|^p} = \gamma > 0,
\end{equation}
we can obtain
\begin{equation}
p = lim_{k\to \infty}\frac{log |x_{k+1} - \alpha | - log \gamma}{|x_{k} - \alpha|} \approx lim_{k\to \infty}\frac{log |x_{k+1} - \alpha |}{|x_{k} - \alpha|}.
\end{equation}
If $p=1$ the convergence is linear, if $p=2$ the convergence is quadratic and so on. 
In our case we consider $\alpha$ as the minimum error obtained by the optimization algorithm for each problem.
As we expect from the analysis we made, \ref{L-BFGSConv} for L-BFGS, \ref{PBM-Convergence} for Proximal Bundle method and \ref{MDA-convergence} for Momentum Descent Approach, the rate of convergence of each algorithms developed respect what we state above.
