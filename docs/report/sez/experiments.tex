...
\subsubsection{Error with respect to the minimum $f^*$}
\label{sec:residual_error}
The error with respect to the minimum $f^*$ for each iteration is obtained by the following formula: $ f(x_k) - f^*$ where  $f(x_{k})$ is the value obtained at the iteration $k$ plus the  regularization term if it is used and $f^*$ is the optimal value reached by the specific configuration of the problem (that includes the regularization term if it is used). The plots of the residual value obtained from the configurations in the table \ref{tab:nets_comp} are shown below. The residual curves obtained are shown below; an enlargement for each of them put to the side.
