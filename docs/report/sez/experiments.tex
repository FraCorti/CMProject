...
To compute the rate of convergence we can derive an alternative faster and rapid procedure. Considering that
\begin{equation}
...
\end{equation}
If $p=1$ the convergence is linear and if $p=2$ the convergence is quadratic. As we can observe in figure \ref{CR-Monk2}, L-BFGS reached a superlinear rate of convergence.  In our case, we consider $\alpha$ as the minimum error obtained by the optimization algorithm for each problem.
