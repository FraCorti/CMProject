...
To compute the rate of convergence for each algorithm, we consider $\alpha$ as the minimum residual obtained by the optimization algorithm and $f(x_{k})$ as the residual obtained at the iteration $k$ plus the  regularization term used. If exists a real positive constant $\gamma$ such that
\begin{equation}
\label{eq:rofFirst}
lim_{k\to \infty}\frac{|f(x_{k+1}) - \alpha |}{|f(x_{k}) - \alpha|^p} = \gamma > 0,
\end{equation}
we can state that the sequence of values $f(x_{k})$ produced by the algorithm has an order of convergence $p$. In our case, we can derive an alternative faster and rapid procedure. Considering the equation \ref{eq:rofFirst} we can obtain the following formula: 
\begin{equation}
...
\begin{figure}[H]
	...
	\begin{minipage}[t]{0.5\linewidth}
		...
		\subcaption{Accuracy}
	\end{minipage}
	\caption{Converge rate comparison Monk 2}
	\label{CR-Monk2}
\end{figure}
\begin{figure}[H]
	\centering
	...
	\begin{minipage}[t]{0.5\linewidth}
	    ...
	    \subcaption{Accuracy}
	\end{minipage}
	\caption{Converge rate comparison Monk 3}
	\label{CR-Monk3}
\end{figure}