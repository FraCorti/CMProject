...
We now describe the updating processes. An iteration \textit{k}, the current iterate $x_{k}$ and the set of vector pairs is given by $\{s_{i},y_{i}\}$ for $i=k-m, ..., k-1$. We first provide a sparse symmetric ans positive definite Hessian approximation $H^0_{k}$ which approximates the inverse Hessian of $f$ 
...
\end{aligned}
\end{equation}
From this expression and given the number $m$ of BFGS corrections that are to be kept we can derive a recursive procedure to compute the product $H_{k}\nabla f_{k}$ efficiently.
