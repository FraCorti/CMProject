...
In the Broyden–Fletcher–Goldfarb–Shanno algorithm (BFGS) each step use the information given by the gradient $\nabla f_{k}$ and the approximated Hessian $H_{k}\textrm{ (or }B_{k})$ to minimize  a function updating parameters $x_{k}$. Each step has the form: 
\begin{equation}
...
We first choose some initial Hessian approximation $H^0_{k}$ and then we find $H_{k}$ with
